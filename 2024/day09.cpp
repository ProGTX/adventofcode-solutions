...
constexpr filesystem_t compact_nonfragment(filesystem_t filesystem) {
  ...
  for (auto rit = filesystem.rbegin(); rit != rend; ++rit) {
    ...
    // Find the leftmost empty range that can fit the current range with a
    // single forward scan tracking the current run length, rather than
    // re-checking every overlapping window of size current_size from
    // scratch (this used std::search before, which does exactly that)
    iterator run_start = begin;
